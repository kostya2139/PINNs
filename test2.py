...
for epoch in range(epochs):
    ...
    if epoch % 100 == 0:
        print(f"Epoch {epoch}, Loss: {loss.numpy():.4e} and secondloss {np.linalg.norm(model.predict(np.hstack((X_flat, T_flat))) - np.sin(np.pi*X_flat)*np.exp(-np.pi**2 * alpha *T_flat))}")
...
print(np.linalg.norm(u_pred - np.sin(np.pi*X_flat)*np.exp(-T_flat)))
